...
-- =============================================================
INSERT INTO `announcement` (`title`, `content`, `type`, `status`, `pinned`, `is_force`, `min_seconds`, `scroll`, `created_by`)
SELECT tmp.title, tmp.content, tmp.type, tmp.status, tmp.pinned, tmp.is_force, tmp.min_seconds, tmp.scroll, tmp.created_by FROM (
  SELECT '欢迎使用闲小鱼' AS title,
         '闲小鱼是一个 C2C 二手交易平台，让闲置流转起来。\n\n请先阅读《平台公约》，遵守交易规则，共同维护良好的交易环境。' AS content,
         1 AS type, 1 AS status, 1 AS pinned, 1 AS is_force, 5 AS min_seconds, 1 AS scroll, 4 AS created_by
  UNION ALL
  SELECT '平台公约',
         '1. 如实描述商品，禁止虚假信息与违规商品。\n2. 交易自愿，请勿线下脱离平台交易。\n3. 文明沟通，禁止辱骂、骚扰等不良行为。\n4. 违规商品将被下架，情节严重者封禁账号。',
         2, 1, 0, 0 AS is_force, 0 AS min_seconds, 1 AS scroll, 4 AS created_by
  UNION ALL
  SELECT '交易提示',
         '支付与收货请务必在平台内完成，谨防线下交易诈骗。',
         3, 1, 0, 0 AS is_force, 0 AS min_seconds, 1 AS scroll, 4 AS created_by
) tmp
